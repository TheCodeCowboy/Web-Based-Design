          ...
          <tr>
			...
			if($addQuery_run = mysqli_query($conn, $addQuery))
			{
				while($addRow = mysqli_fetch_assoc($addQuery_run))
				{
					...
					if($sub_query_run = mysqli_query($conn, $sub_query))
					{
						while($subRow = mysqli_fetch_assoc($sub_query_run))
						{
							
							if($addRow['ID'] == $subRow['subject'])
							{
			...
			?>
			  ...
			  <?php 
			  //<a href="editskills.php"><button class="editSkillsButton">Save</button></a> 
			  ?>
              <td>
              </td>
          </tr>
