...
if(isset($_POST['title'], $_POST['recip'], $_POST['message']))
{
        ...
        if($_POST['title']!='' and $_POST['recip']!='' and $_POST['message']!='')
        {
				...
				$query = "select count(id) as recip, id as recipid, (select count(*) from pm) as npm from user where username='".$recip."'";
                $dn1 = mysqli_fetch_array(mysqli_query($conn, "select count(username) as recip, username as recipid, (select count(*) from pm) as npm from user where username='".$recip."'"));
                if($dn1['recip']==1)
                {
                        //We check if the recipient is not the actual user
                        if($dn1['recipid']!=$_SESSION['user_name'])
                        {
                                $id = $dn1['npm']+1;
                                //We send the message
                                if(mysqli_query($conn, 'insert into pm (id, id2, title, user1, user2, message, timestamp, user1read, user2read)values("'.$id.'", "1", "'.$title.'", "'.$_SESSION['user_name'].'", "'.$dn1['recipid'].'", "'.$message.'", "'.time().'", "yes", "no")'))
                                {
?>
